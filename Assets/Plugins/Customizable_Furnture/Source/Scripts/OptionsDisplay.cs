    ...
    public void GOCleanup(){
        ...
        if (groupPivotsChilds != null) {
            for (int i=1;i<groupPivotsChilds.Length;i++){
                ...
                if (groupPivotsChilds[i].childCount==0)
                {
                    DestroyImmediate(groupPivotsChilds[i].gameObject);
                }
            }
        }
        if (groupPivots!=null)
        {
            DestroyImmediate (groupPivots.gameObject);
        }
        //digital clock second part
        if (digitalClockObject!=null)
        {
            digitalClockObject.transform.parent = this.gameObject.transform;
        }
        //end of second part
